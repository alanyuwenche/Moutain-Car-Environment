...
class SumTree(object):
    ...
    def update(self, tree_index, priority):
        ...
        while tree_index !=0 :
            tree_index = (tree_index - 1) // 2 # divided by 2 and remove decimal
...
class Memory(object):
    ...
    a = 0.6 # [0 ~ 1] convert the importance of TD error to proiority
    b = 0.4 # importance sampling, from initial value increasing to 1
    b_del = 0.001
    max_error = 1.0
    ...
    def sample(self, n):
        ...
        #b_ISWeights = np.empty((n, 1))#20200220
        #b_idx, memory_b, b_ISWeights = np.empty((n,), dtype=np.int32), np.empty((n, self.tree.data[0].size)), np.empty((n, 1))
        ...
        for i in range(n):
            ...
            #prob = p / self.tree.total_p   cccc
            # Weight update factor depends on sampling probability of the experience and b
            b_ISWeights[i, 0] = np.power(sampling_probability/p_min, -self.b) #20200220
